...
export default function About({ lang }: AboutProps) {
  ...
  const [photoUrls] = useState<string[]>(() => {
    const defaultPhotos = [
      '/src/assets/images/regenerated_image_1780739726126.png',
      '/src/assets/images/regenerated_image_1780739735044.png',
      '/src/assets/images/regenerated_image_1780739720005.png'
    ];
    try {
      const stored = localStorage.getItem('profile-photos-array');
      if (stored) {
        const parsed = JSON.parse(stored);
        return parsed.length > 0 ? parsed : defaultPhotos;
      }
      const old = localStorage.getItem('profile-photo-url');
      return old ? [old] : defaultPhotos;
    } catch {
        ...
        <div className="flex flex-col md:flex-row gap-12 lg:gap-16 items-start max-w-4xl mx-auto">
          ...
          <div className="w-full md:w-auto flex-shrink-0 mx-auto md:mx-0">
            ...
            >
              ...
              <div 
                onClick={() => setZoomedPhotoIndex(activePhotoIndex)}
                className="relative aspect-[4/5] bg-zinc-950/80 border border-white/5 overflow-hidden group/photo flex items-center justify-center cursor-zoom-in"
              >
